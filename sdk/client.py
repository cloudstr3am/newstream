...
class NewStream:
    """
    NewStream SDK — drop into any AI product to stream session events.

    Usage:
        from client import NewStream

        stream = NewStream(
            base_url="https://your-newstream-url.com",
            api_key="cloudsek-prod-key-change-this",
            session_id="user-123-session-456"  # use your own session/user id
        )

        stream.token("Hello", model="gpt-4", latency_ms=45)
        stream.tool_call("search", {"query": "news"}, {"results": 5})
        stream.error("Rate limit hit")
        stream.done()
    """
